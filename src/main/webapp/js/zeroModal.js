...
(function(obj) {
    ...
    var _buildModal = function(opt) {
        ...
        if (opt.left != null && opt.top != null) {
            _left = opt.left.replace('px', '');
            _top = opt.top.replace('px', '');
        }

        //// 构建容器
        var _container = $('<div zero-unique-container="' + opt.unique + '" class="zeromodal-container" id="zeroModalContainer" style="z-index:' + _tmp_last_zindex + ';width:' + _width + 'px;height:' + _height + 'px;left:' + _left + 'px;top:' + (opt.transition ? _top - 50 : _top) + 'px"></div>');
        ...
        var _body = $('<div zero-unique-body="' + opt.unique + '" class="zeromodal-body" id="zeroModalBody" style="height:' + _bodyHeight + 'px;"></div>');
        ...
        if (!opt.url) {
            ...
            if(opt.formModel){
            	//构建参数
            	var _form=$('<form id="form_1"  class="form-horizontal" role="form"  action="'+opt.formModel.url+'"></form>');
            	
            	for(var i=0;i<opt.formModel.cols.length;i++){
            		_div_append(_form);
            	}   
            	
            	if(opt.formModel.cols.length!=0){
            		_form.append('<input type="submit" value="查看"/>');
            	}
            	_form.load(function() {
            ...
            if (opt.iframe) {
                var _iframe = $('<iframe  id="zeroModalBodyIFrame" src="' + opt.url + '" class="zeromodal-frame"></iframe>');
    ...
    var _div_append=function(container){
    	container.append('<div class="form-group">');
    	container.append('  <label for="inputEmail3" class="col-sm-2 control-label">Email</label>');
    	container.append('  <div class="col-sm-10">');
    	container.append('<input type="email" class="form-control" id="inputEmail3" placeholder="Email">');
